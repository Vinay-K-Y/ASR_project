...
# --- NEW: A lock to prevent print statements from overlapping ---
...
def load_known_speakers(known_dir="known_speakers"):
    ...
    if not os.path.exists(known_dir):
        print(f" Warning: Known speakers directory '{known_dir}' not found.")
        return speaker_embeddings
        
    print(f" Loading known speakers from '{known_dir}'...")
        ...
        if os.path.isdir(person_path):
            for file in glob.glob(os.path.join(person_path, "*.wav")):
                try:
                    ...
                    speaker_embeddings[person_dir].append(normalize(embed))
                except Exception as e:
                    print(f" Error loading {file}: {e}")
    return speaker_embeddings

def identify_known_speaker(embed, known_speakers):
    embed = normalize(embed)
    best_match = None
    highest_similarity = 0
    for name, embed_list in known_speakers.items():
...
def draft_transcription_thread(model, device, lock):
    ...
    while not stop_event.is_set():
        try:
            ...
            # --- KEY CHANGE: Acquire lock before printing ---
            with lock:
...
def final_transcription_thread(model, device, known_speakers, lock):
    print("Final transcription & diarization thread started.")
    unknown_speaker_embeddings = defaultdict(list)
    unknown_counter = 1
    ...
    while not stop_event.is_set():
        try:
            ...
            speaker_name = identify_known_speaker(embed, known_speakers)
            
            if speaker_name is None:
                matched_label = identify_known_speaker(embed, unknown_speaker_embeddings)
                if matched_label:
                    speaker_name = matched_label
                else:
                    speaker_name = f"Speaker {unknown_counter}"
                    unknown_counter += 1
                unknown_speaker_embeddings[speaker_name].append(normalize(embed))
...
if __name__ == "__main__":
    ...
    print(f"All models loaded. {len(known_speakers)} known speakers identified.")
    ...
    threads = [
        ...
        threading.Thread(target=final_transcription_thread, args=(final_model, device, known_speakers, print_lock), daemon=True)
    ]
